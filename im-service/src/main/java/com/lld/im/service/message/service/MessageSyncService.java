...
public class MessageSyncService {
     ...
     * @return void
     * @author 消息已读.更新会话的seq, 通知在线的同步端发送指定command, 发送已读回执通知对方(消息发起方)我已读
     **/
    public void readMark(MessageReadedContent messageContent) {
        ...
        messageProducer.sendToUser(messageContent.getToId(), MessageCommand.MSG_READED_RECEIPT,
                messageReadedPack, messageContent.getAppId());
    }
...
//        BeanUtils.copyProperties(messageReadedContent,messageReadedPack);
    ...
    public void recallMessage(RecallMessageContent content) {
        ...
        if (content.getConversationType() == ConversationTypeEnum.P2P.getCode()) {
            ...
            recallAck(pack, ResponseVO.successResponse(), content);
            //发送给同步端
            messageProducer.sendToUserExceptClient(content.getFromId(), MessageCommand.MSG_RECALL_NOTIFY,
                    pack, content);
            //发送给群聊所有成员
